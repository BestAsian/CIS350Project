...
BLACK = (0, 0, 0)
RED = (255, 0, 0)
GREEN = (0, 255, 0)  # Adding green for the zero slot (optional)
...
WHEEL_RADIUS = 300
BALL_RADIUS = 12
NUM_SLOTS = 37  # Including a green slot for '0'
...
bet_amount = 0
last_bet_info = ""
bet_type = None  # Added to track the type of bet made
...
def draw_wheel(angle):
    ...
    for i in range(NUM_SLOTS):
        ...
        # Assigning green color to the '0' slot and alternating red and black for others
        if i == 0:  # Assuming the first slot is '0' and it's green
            color = GREEN
        else:
...
def handle_buttons(mouse_pos):
    # Handle button rendering and click detection
...
def calculate_slot(angle):
    # Calculate the slot number based on the final ball angle
    degrees_per_slot = 360 / NUM_SLOTS
    normalized_angle = math.degrees(angle) % 360
    slot_number = int(normalized_angle / degrees_per_slot)  # Corrected formula
    return slot_number
    ...
    while running:
        ...
        if ball_spinning:
            ...
            if ball_spin_speed < 0.05:
                ...
                bet_amount = 0  # Reset bet amount after resolving the bet
                bet_type = None  # Reset bet type

        screen.fill(BLACK)
        draw_wheel(0)  # Keep the wheel stationary for simplicity
